    ...
    <div className="flex min-h-screen items-center justify-center bg-neutral-950 px-4">
      <form onSubmit={onSubmit} className="w-full max-w-md space-y-4">
        ...
        <div>
          ...
          <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-white/10 bg-neutral-900 px-4 py-4 text-white outline-none focus:border-white/30" required />
        </div>
        <div>
        ...
        <p className="text-center text-sm text-neutral-400">
          Already have an account? <a href="/login" className="text-white underline underline-offset-2">Sign in</a>
        </p>
      </form>
    </div>
  );
